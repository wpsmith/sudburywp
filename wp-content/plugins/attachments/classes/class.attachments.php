        ...
        function meta_box_markup( $post, $metabox ) {
            ...
            <script type="text/javascript">
                jQuery(document).ready(function($){
                    ...
                    $element.on( 'click', '.perm-delete-attachment a', function( event ) {

                        event.preventDefault();

                        targetAttachment = $(this).parents('.attachments-attachment');

                        if ( confirm("You are about to PERMANENTLY delete the document. \n 'Cancel' to stop, 'OK' to delete.") ) {
                            var attachment_id = targetAttachment.find('input.attachments-track-id').val()

                            var nonce = $('#sudbury-delete-attachment-nonce').text();

                            $.post('admin-ajax.php?action=sudbury_api&delete_attachment=' + attachment_id + '&_wp_nonce=' + nonce, {}, function () {
                                targetAttachment.find('.attachment-meta').fadeOut(125);
                                targetAttachment.css('min-height',0).animate(
                                    {
                                        padding: 0,
                                ...
                                setTimeout(function () {
                                  alert("The item has been deleted, please be sure to save this post before leaving.");
                                }, 1000);
                            });
                        }
                    } );
                ...
                <div class="attachments-attachment attachments-attachment-<?php echo esc_attr( $instance ); ?>">
                    ...
                    <div class="attachment-meta media-sidebar">
                        ...
                        <div class="attachment-details attachment-info details">
                            ...
                            <?php endif; ?>
                            <div class="edit-attachment"><a target="_blank" href="<?php echo admin_url('post.php?post=' . ( isset( $attachment->id ) ? $attachment->id : '{{ attachments.id }}' ) . '&amp;action=edit&amp;referred=true&amp;referrer_post_type=<?php echo esc_attr($this->get_post_type()); ?>'); ?>"><?php _e( 'Edit', 'attachments' ); ?></a></div>
                            <div class="perm-delete-attachment"><a style="color:#a00;" href="#"><?php _e( 'Delete', 'attachments' ); ?></a></div>
                            <div class="delete-attachment"><a href="#"><?php _e( 'Unlink', 'attachments' ); ?></a></div>
                        </div>
