...
<div class="wrap">
    ...
    <?php
        ...
        if ( isset( $_GET['migrate'] ) ) {
            ...
            <?php endif; ?>
            <div id="attachment-types">
              <h2>Attachment Types</h2>
              <p>This is an extention of the attachments plugin that specifies a type attribute for each document via the attachments tool</p>

              <div class="wrap">
                <h2 class="nav-tab-wrapper">
                  <a class="nav-tab nav-tab-active">Post</a>
                  <a class="nav-tab">Meeting</a>
                </h2>
                <section>
                  Tab 1
                </section>
    ...
    <script type="text/javascript">
        ...
        (function($) {
          $(document).on( 'click', '.nav-tab-wrapper a', function() {
            $('#attachment-types section').hide();
            $('#attachment-types section').eq($(this).index()).show();
            $(this).siblings().removeClass('nav-tab-active')
            $(this).addClass('nav-tab-active')

            return false;
          });
          $(document).on( 'click', '#attachment-types button.add-type', function() {
            ...
            var template = _.template($('script#tmpl-attachment-types').html());
            console.log(template);
            $(this).siblings('.nav-tab-active').find('.attachment-types-list').append(template)
            $(this).addClass('nav-tab-active')
    ...
    <script type="text/template" id="tmpl-attachment-types">
      <?php create_attachment_types_selector(); ?>
    </script>
