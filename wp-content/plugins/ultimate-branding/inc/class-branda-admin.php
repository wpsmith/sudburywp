...
if ( ! class_exists( 'Branda_Admin' ) ) {
	...
	class Branda_Admin extends Branda_Base {
		...
		public function add_plugin_action_links( $actions, $plugin_file, $plugin_data, $context ) {
			...
			$links = [
				'settings' => '<a href="'. $settings_url .'">' . __( 'Settings', 'ub' ) . '</a>',
				'docs' => '<a href="https://premium.wpmudev.org/docs/wpmu-dev-plugins/branda/?utm_source=branda&utm_medium=plugin&utm_campaign=branda_pluginlist_docs" target="_blank">' . __( 'Docs', 'ub' ) . '</a>',
			];
			if ( ! Branda_Helper::is_member() ) {
				$links['renew'] = '<a href="https://premium.wpmudev.org/?utm_source=branda&utm_medium=plugin&utm_campaign=branda_pluginlist_renew" target="_blank" style="color: #8D00B1;">' . __( 'Renew Membership', 'ub' ) . '</a>';
			}
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( $branda_plugin_file === $plugin_file ) {
				$row_meta = array(
					...
					'roadmap' => '<a href="https://premium.wpmudev.org/roadmap/" target="_blank">' . esc_html__( 'Roadmap', 'ub' ) . '</a>',
				);
