(function ($) {
	...
	window.Branda.Menu_Item_Container = function () {
		...
		function get_visible_items() {
			return filter_visible_items(items);
		}

		/**
		 * @type {Branda.Menu_Item[]}
		 */
		function filter_visible_items(filterable) {
		...
		function set_hide_button_visibility(show_hide_button) {
			if (show_hide_button) {
		...
		function handle_item_selection_status_changed() {
			...
			var visible_items = filter_visible_items(selected_items);
			set_hide_button_visibility(!!visible_items.length);
		...
		function init_select_all_listener() {
			elements.select_all_checkbox.on('change', function () {
				...
				if (is_checked) {
					var visible_item_count = get_visible_items().length;
					set_hide_button_visibility(!!visible_item_count);
				}
			});
		}
		...
		function init_hide_button_listener() {
			elements.hide_button.on('click', function () {
				...
				set_hide_button_visibility(false);
			});
		}
		...
		function init_unhide_button_listener() {
			elements.unhide_button.on('click', function () {
				...
				set_hide_button_visibility(true);
			});
		}
