...
class Sudbury_Post_Options {
	...
	function init() {
		...
		$this->post_types = apply_filters( 'post_options_post_types', array(
			'post',
			'page',
			'service',
			'faq',
	...
	function render() {
		...
		$layout           = sudbury_get_post_layout( $id );
		?>
		<div class="sudbury-metabox">
			...
			<?php if ( 'page' !== get_post_type( $id ) ) : ?>
				<hr>
				<div id="sudbury_layout_container">
					<label for="sudbury_layout">
						<b> Layout </b>
						<select id="sudbury_layout" name="sudbury_layout">
							<option value="normal" <?php selected( 'normal' == $layout ); ?>>Normal - Sidebar Layout</option>
							<option value="full" <?php selected( 'full' == $layout ); ?>>Full - Full Page Layout</option>
						</select>
					</label>
				</div>
			<?php endif; ?>
	...
	function save( $post_id ) {
		...
		if ( isset( $_REQUEST['sudbury_layout'] ) ) {
			$sudbury_layout = sanitize_title( $_REQUEST['sudbury_layout'] );

			update_post_meta( $post_id, 'sudbury_layout', $sudbury_layout );
		}
	}
}
