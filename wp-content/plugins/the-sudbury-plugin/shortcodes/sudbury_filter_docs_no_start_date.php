 ...
 */
class Sudbury_Filter_DocsNoStart {
	...
	function init() {
		...
		add_shortcode( 'filter-docs-no-start', array( &$this, 'shortcode' ) );
	...
	function shortcode( $archiving = false ) {
		$atts = array( //moe hardcoding these so nothing needs to be passed in
			'query_post_mime_type' => 'application*',
			'query_post_type' => 'attachment',
			'query_post_status' => 'inherit',
			'fields'          => 'post_title_link,blogname,terms_document_categories',
			'headers'         => 'Title,Department/Committee,Categories',
			'title'           => 'All Documents',
			'network'         => 'true',
			'mid_link'        => '',
			'mid_link_text'   => '',
		...
		if ( $archiving) {
			$atts['query_meta_key'] = 'sudbury_end_date_timestamp';
			$atts['query_meta_value'] = '__TIME__';
			$atts['query_meta_compare'] = '>';
		}
...
<?php
	...
	$transient_hash = wp_hash( serialize( $atts ) . '|' . get_the_ID() );
		...
		//$mid_link = ($archiving) ? '/all-documents-for-sudbury/' : '/all-documents/archived-docs-by-category/';
		//$mid_text = ($archiving) ? "Click This Link to Exclude Archived Documents." : "Click This Link to Include Archived Documents... Please Be Patient";
...
$start_timestamp = get_post_meta( $post['ID'], 'sudbury_start_date_timestamp', true );
if ( ! $start_timestamp) {
...
new Sudbury_Filter_DocsNoStart();
