...
class Sudbury_Filter_ByCats {
	...
	function init() {
		...
		add_shortcode( 'filter-news-bycat', array( &$this, 'shortcode' ) );
	...
	function shortcode( $archiving = false ) {
							...
							Category: <select name="categorySelect">
								...
								foreach ( $blogs as $blog ) {
									...
									foreach ( $categories as $category ) {
										...
										//if ( ! is_category_hidden($category->term_id) && ($category->slug != 'internal-news') ) {
										//  if ($category->slug != 'internal-news')  {
										$p_args    = array(
		...
		<div class="tablecap">
			<div class="cap">
				<?php echo esc_html( $atts['title'] ); ?> <?php if ( 'true' === $atts['cache'] ) : ?>
			...
			<table cellspacing="0" style="display: table;" class="filter-table">
				...
				foreach ( $posts as $post ) {
					...
					// moe: this code is only needed in the filterable which is live to the public
