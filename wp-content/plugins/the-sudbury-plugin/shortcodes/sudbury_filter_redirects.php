 ...
 *
 * @author     Eddie Hurtig <hurtige@sudbury.ma.us>; edited by Moe Finigan to display news with redirect links.
 * @package    Sudbury
 * @subpackage Shortcodes
 */
class Sudbury_Filter_Redirects {
	...
	function init() {
		$this->excluded_cat_names = apply_filters( 'filterredirects_excluded_cat_names', $this->excluded_cat_names );
		add_shortcode( 'filterredirects', array( &$this, 'shortcode' ) );
	...
	function shortcode( $atts = array(), $content = null ) {
		$defaults = array(
			...
			'mid_link'        => '',
			'mid_link_text'   => '',
			'cache'           => 'false', //for this shortcode, we don't want the cache
			'hide_guests'     => 'true',
		);
	...
	$transient_hash = wp_hash( serialize( $atts ) . '|' . get_the_ID() );
		$cache_file     = wp_upload_dir()['basedir'] . '/cache/filterredirect-' . wp_hash( serialize( $atts ) . '-' . get_the_ID() ) . '.part.html';
		...
		// We like to cache this shortcode and we do not want to reload the cache manually then lets look into pulling from the cache
			...
			if ( ! file_exists( $cache_file ) || false === get_transient( 'filterredirect-' . $transient_hash ) ) {
				// Lets just make sure that we are not trying to rebuild the cache right now
				if ( $started_job = ( ! get_transient( 'filterredirect-building-' . $transient_hash ) ) ) {
		...
		set_transient( 'filterredirect-building-' . $transient_hash, true, 600 );
...
$enabled =  get_post_meta( $post['ID'], 'sudbury_redirect_enabled', true );
if ($enabled == '1') {
					...
					foreach ( $fields as $field )  {
						if ( strstartswith( 'meta_', $field ) ) {
							...
							$field_value = '<a href="' . get_permalink( $post['ID'] ) . '">' . $post['post_title'] . '</a><br/>Redirect URL: ' . esc_url($url);
							//$field_value = '<a href="' . get_permalink( $post['ID'] ) . '">' . get_permalink( $post['ID'] ) . '</a><br/>Redirect URL: ' . $url;
		...
		// Cache this puppy
		if ( 'true' == $atts['cache'] || $cachebuster ) {
			set_transient( 'filterredirect-' . $transient_hash, 1, 3600 * 24 );
			// For soft caching
		...
		delete_transient( 'filterredirect-building-' . $transient_hash );
