<?php
/**
 * Display All News - from every site. Has a category drop-down to select by category
 *
 * @author     Eddie Hurtig <hurtige@sudbury.ma.us>, Moe Finigan <moe@moegood.com>
 * @package    Sudbury
 * @subpackage Shortcodes
 */
class Sudbury_Filter_SudNews_ByCats {
	...
	function init() {
		$this->excluded_cat_names = apply_filters( 'filternewsbycat_excluded_cat_names', $this->excluded_cat_names );
		add_shortcode( 'filter-sudnews-bycat', array( &$this, 'shortcode' ) );

	}


	function shortcode( $archiving = false ) {
		$p_status = (!$archiving) ? 'publish' : array('publish', 'public-archive');

		$atts = array( //moe hardcoding these so nothing needs to be passed in
			'query_post_type' => 'post',
			'fields'          => 'post_title_link,post_date,blogname,terms_category',
			'headers'         => 'Title,Date Published,Department/Committee, Categories',
			'title'           => 'All News',
			'network'         => 'true',
			'mid_link'        => '',
			'mid_link_text'   => '',
			'cache'           => 'true',  /* should be true for live site public use */
			'hide_guests'     => 'true',
			'colstyles'		  => 'width:30%,width:20%,width:20%,width:30%',
			'query_posts_per_page' => '-1',
			'query_category_name' => '',
		);
		//if ($archiving) {
		//	$atts['query_post_status'] = $p_status;
		//}		
...
$i=0; 
	foreach ( $categories as $category ) {
	  // don't show hidden categories in selection drop-down
	  if ( (!is_category_hidden($category->term_id)) && ( !in_array($category->name, $excluded_cat_names)) ) { 
		$p_args=array(
...
<?php
		if ( isset($_POST['categorySelect']) ) {
			$atts['query_category_name'] = $active_cat == 'all-categories' ? '' : $_POST['categorySelect'];
			?>
			<div class="space"></div>
         <?php
		}
	...
	$transient_hash = wp_hash( serialize( $atts ) . '|' . get_the_ID() );
		...
		//}
		// change message according to whether you are displaying the archived news articles or just current news articles
		$mid_link = ($archiving) ? '/all-news/' : '/all-news/archived-news-by-category/';
		$mid_text = ($archiving) ? "Click This Link to Exclude Archived News Articles." : "Click This Link to Include Archived News Articles... Please Be Patient";
...
new Sudbury_Filter_SudNews_ByCats();
