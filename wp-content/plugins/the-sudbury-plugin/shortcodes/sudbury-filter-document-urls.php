 ...
 */
class Sudbury_Filter_Document_Urls {
	...
	function init() {
		$this->excluded_cat_names = apply_filters( 'filter-doc-urls_excluded_cat_names', $this->excluded_cat_names );
		add_shortcode( 'filter-document-urls', array( &$this, 'shortcode' ) );
				...
				foreach ( $posts as $post ) {
					...
					foreach ( $fields as $field ) {
						if ( strstartswith( 'meta_', $field ) ) {
							...
							$doc_url = wp_get_attachment_url( $post['ID'] );
							$field_value = '<a href="' . get_permalink( $post['ID'] ) . '">' . $post['post_title'] . '</a><br/>File URL: ' . esc_html($doc_url);
...
new Sudbury_Filter_Document_Urls();
