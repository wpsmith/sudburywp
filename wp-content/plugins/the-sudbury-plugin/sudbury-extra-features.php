...
$sudbury_edit_feature_page_hook = null;
if ( ! defined( 'LEGACY_SECRET_HASH_SALT' ) ) {
	define ( 'LEGACY_SECRET_HASH_SALT', 'ffwZxV3HrHQTAWktsJCc4T|znQlrczoTJ056dZ iTcgI_|GJw^th MHFt740' );
}
function sudbury_add_extra_feature_menus() {
	...
	if ( $options['features'] ) {
		...
		foreach ( $options['features'] as $slug => $feature ) {
			if ( sudbury_is_feature_enabled( $slug, $feature ) ) {
				if ( isset( $feature['allowed-users'] ) && is_array( $feature['allowed-users'] ) && ! empty( $feature['allowed-users'] ) && ! in_array( get_current_user_id(), $feature['allowed-users'] ) && ! empty( $feature['source'] ) ) {
					continue;
				}
				add_menu_page( $feature['page-title'], $feature['menu-title'], $feature['capability'], "sudbury_extra_feature_$slug", "sudbury_extra_feature_page", sudbury_feature_icon_url( $feature['icon'] ), $feature['menu-position'] );
			}
		}
	}
	...
	if ( is_super_admin() ) {
		...
		add_menu_page( "Sudbury Department Features", "Dept. Features", 'manage_network', 'sudbury-extra-feature-admin', 'sudbury_extra_feature_admin', 'dashicons-hammer', 43 );
		$sudbury_edit_feature_page_hook = add_submenu_page( 'sudbury-extra-feature-admin', "Department Feature", "Add Feature", 'manage_network', 'sudbury-edit-feature', 'sudbury_edit_feature_page' );
...
$GLOBALS['sudbury_auth_timestamp'] = time();

function sudbury_extra_features_source( $feature ) {
	$username     = wp_get_current_user()->user_login;
	$timestamp    = $GLOBALS['sudbury_auth_timestamp'];
	$ip           = 'N/A';
	...
	$sha256 = hash( "sha256", $username . LEGACY_SECRET_HASH_SALT . $timestamp . $ip );

	return sprintf( $feature['source'], sudbury_get_blog_slug(), sudbury_get_the_site_name(), sudbury_get_site_type(), get_option( 'sudbury_board_membership_key', '' ), apply_filters( 'sudbury_resolve_to_webeditor_name', sudbury_get_blog_slug() ), apply_filters( 'sudbury_resolve_to_webeditor_long_name', sudbury_get_the_site_name() ), $username, $sha256, $timestamp );

}

function sudbury_extra_feature_page() {
	...
	<iframe allowtransparency src="<?php echo esc_url( $source ); ?>" id="source-frame" style="position: absolute; top:0; bottom: 41px; left:0px; right:0; width:100%; height: 100%;" onerror="alert('error')">
						...
						<td>
							...
							<span class="description">This is the fully qualified URL or Root-relative url of the application you want to access.  for example for paging it is <code>http://svrinweb/paging</code><br>
								<ul>
									...
									<li>Use <code>%8$s</code> for the SHA256 Authentication Hash
										<code>md5(USERNAME + LEGACY_SECRET_HASH_SALT + TIMESTAMP)</code>
									</li>
									<li>Use <code>%9$s</code> for the Timestamp (For use with the Authentication Hash)
									</li>
								</ul>
							</span>
						</td>
