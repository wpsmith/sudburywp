<?php
/*
Plugin Name: (Sudbury Version) Recent Custom Posts Type Widget
...
Note - can't update but this is the WDP ID: 226

Eddie's Changelog for town of Sudbury: https://bitbucket.org/sudbury/sudburywp/diff/wp-content/plugins/recent_custom_post_type/recent-custom-post-type.php?diff1=0fad1881061a&diff2=1a2996bd57ac6de674c95a7f750f5dea9da0e193&at=master
*/
...
class RcptWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'rcpt', 'Recent Custom Posts Type Widget', $widget_ops );
	}
...
class RcptShortcode {
	...
	function __construct() {
		...
		add_shortcode( 'all-posts-cpt', array( &$this, 'shortcode' ) );
	}
	...
	function shortcode( $args ) {
		...
		$titles_as_links = $instance['titles_as_links'] == 'true';
		$show_body       = $instance['show_body'] == 'true';
		$show_thumbs     = (int) $instance['show_thumbs'] == 'true';
		$show_dates      = (int) $instance['show_dates'] == 'true';
...
//            'order' => $order_dir,
		...
		$query = new Network_Query( $query_args );

		if ( $query->have_posts() ) {
			if ( $title ) {
				echo $title;
			} // allow html in title for now
...
$RcptShortcode = new RcptShortcode();
