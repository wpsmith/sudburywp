<?php
/*
Plugin Name: (Sudbury Version) Easy WP SMTP
Version: 10.0.0
...
if ( ! function_exists( 'swpsmtp_admin_default_setup' ) ) {
	function swpsmtp_admin_default_setup() {
		add_submenu_page(
			'settings.php', __( 'Easy WP SMTP', 'easy_wp_smtp' ), __( 'Easy WP SMTP', 'easy_wp_smtp' ), 'manage_network', 'swpsmtp_settings', 'swpsmtp_settings'
		);
...
if ( ! function_exists( 'swpsmtp_init_smtp' ) ) {
	function swpsmtp_init_smtp( $phpmailer ) {
		...
		$swpsmtp_options = get_site_option( 'swpsmtp_options' );
...
if ( ! function_exists( 'swpsmtp_settings' ) ) {
	function swpsmtp_settings() {
		...
		<div class="swpsmtp-mail wrap" id="swpsmtp-mail">
			...
			<div class="update-nag">Please visit the
				<a target="_blank" href="http://wp-ecommerce.net/easy-wordpress-smtp-send-emails-from-your-wordpress-site-using-a-smtp-server-2197">Easy WP SMTP</a> documentation page for usage instructions.
			</div>
			...
			<div id="swpsmtp-settings-notice" class="updated fade" style="display:none">
				<p>
					<strong><?php _e( "Notice:", 'easy_wp_smtp' ); ?></strong> <?php _e( "The plugin's settings have been changed. In order to save them please don't forget to click the 'Save Changes' button.", 'easy_wp_smtp' ); ?>
				</p>
			</div>
			...
			<form id="swpsmtp_settings_form" method="post" action="">
				<table class="form-table">
					...
					<tr class="ad_opt swpsmtp_smtp_options">
						...
						<td>
							<label for="swpsmtp_smtp_type_encryption_1"><input type="radio" id="swpsmtp_smtp_type_encryption_1" name="swpsmtp_smtp_type_encryption" value='none' <?php if ( 'none' == $swpsmtp_options['smtp_settings']['type_encryption'] ) {
									echo 'checked="checked"';
								} ?> /> <?php _e( 'None', 'easy_wp_smtp' ); ?></label>
							<label for="swpsmtp_smtp_type_encryption_2"><input type="radio" id="swpsmtp_smtp_type_encryption_2" name="swpsmtp_smtp_type_encryption" value='ssl' <?php if ( 'ssl' == $swpsmtp_options['smtp_settings']['type_encryption'] ) {
									echo 'checked="checked"';
								} ?> /> <?php _e( 'SSL', 'easy_wp_smtp' ); ?></label>
							<label for="swpsmtp_smtp_type_encryption_3"><input type="radio" id="swpsmtp_smtp_type_encryption_3" name="swpsmtp_smtp_type_encryption" value='tls' <?php if ( 'tls' == $swpsmtp_options['smtp_settings']['type_encryption'] ) {
									echo 'checked="checked"';
								} ?> /> <?php _e( 'TLS', 'easy_wp_smtp' ); ?></label><br />
					...
					<tr class="ad_opt swpsmtp_smtp_options">
						...
						<td>
							<label for="swpsmtp_smtp_autentication"><input type="radio" id="swpsmtp_smtp_autentication" name="swpsmtp_smtp_autentication" value='no' <?php if ( 'no' == $swpsmtp_options['smtp_settings']['autentication'] ) {
									echo 'checked="checked"';
								} ?> /> <?php _e( 'No', 'easy_wp_smtp' ); ?></label>
							<label for="swpsmtp_smtp_autentication"><input type="radio" id="swpsmtp_smtp_autentication" name="swpsmtp_smtp_autentication" value='yes' <?php if ( 'yes' == $swpsmtp_options['smtp_settings']['autentication'] ) {
									echo 'checked="checked"';
								} ?> /> <?php _e( 'Yes', 'easy_wp_smtp' ); ?></label><br />
			...
			<div class="updated fade" <?php if ( empty( $result ) ) {
				echo "style=\"display:none\"";
			} ?>>
...
add_action( 'network_admin_menu', 'swpsmtp_admin_default_setup' );
