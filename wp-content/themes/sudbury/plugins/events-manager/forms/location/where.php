...
<div id="em-location-data" class="em-location-data">
	<table class="em-location-data">
		...
		<tr class="em-location-data-town">
			...
			<td>
				<input id="location-town" type="text" name="location_town" value="<?php echo esc_attr( $EM_Location->location_town == '' ? 'Sudbury' : $EM_Location->location_town, ENT_QUOTES ); ?>" /> <?php echo $required; ?>
			</td>
		</tr>
		<tr class="em-location-data-state">
			<th><?php _e( 'State/County:', 'dbem' ) ?>&nbsp;</th>
			<td>
				<input id="location-state" type="text" name="location_state" value="<?php echo esc_attr( $EM_Location->location_state == '' ? 'MA' : $EM_Location->location_state, ENT_QUOTES ); ?>" />
			</td>
		</tr>
		<tr class="em-location-data-postcode">
			<th><?php _e( 'Postcode:', 'dbem' ) ?>&nbsp;</th>
			<td>
				<input id="location-postcode" type="text" name="location_postcode" value="<?php echo esc_attr( $EM_Location->location_postcode == '' ? '01776' : $EM_Location->location_postcode, ENT_QUOTES ); ?>" />
			</td>
		</tr>
		...
		<tr class="em-location-data-country">
			<th><?php _e( 'Phone:', 'dbem' ) ?>&nbsp;</th>
			<td>
				<input id="location-phone" type="text" name="sudbury_location_phone" value="<?php echo esc_attr( get_post_meta( $EM_Location->post_id, 'sudbury_location_phone', true ) ); ?>" />
				<?php wp_nonce_field( 'sudbury_location_phone', 'sudbury_location_phone_nonce' ); ?>
				<br>
				<i>This is an optional phone number to show with this building's contact information</i>
			</td>
		</tr>
	</table>
	<?php if ( get_option( 'dbem_gmap_is_active' ) ) {
