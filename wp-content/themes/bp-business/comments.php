...
?>
	<div id="comments">
		...
		<h3>
			<?php printf( _n( '1 response to %2$s', '%1$s responses to %2$s', $num_comments, TEMPLATE_DOMAIN ), number_format_i18n( $num_comments ), '<em>' . get_the_title() . '</em>' ) ?>
		</h3>
...
<?php else : ?>
	...
	<?php if ( pings_open() && !comments_open() && ( is_single() || is_page() ) ) : ?>
		<p class="comments-closed pings-open">
			<?php printf( __( 'Comments are closed, but <a href="%1$s" title="Trackback URL for this post">trackbacks</a> and pingbacks are open.', TEMPLATE_DOMAIN ), get_trackback_url() ) ?>
		</p>
...
<?php if ( !empty( $num_trackbacks ) ) : ?>
	<div id="trackbacks">
		<h3><?php printf( _n( '1 trackback', '%d trackbacks', $num_trackbacks, TEMPLATE_DOMAIN ), number_format_i18n( $num_trackbacks ) ) ?></h3>
